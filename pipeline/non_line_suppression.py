...
def non_line_suppression(img: np.ndarray, edges: np.ndarray, dx: Optional[np.ndarray]=None,
    ...
    # lo-hi-lo edges.
    filtered_directions = np.abs(cv2.boxFilter(directions, cv2.CV_32F, (10, 10), normalize=True, borderType=cv2.BORDER_CONSTANT))
    filtered_directions = 1 - filtered_directions / filtered_directions.max()

    _, suppression = cv2.threshold(np.abs(filtered_directions), .7, 1, cv2.THRESH_BINARY)
    edges_suppressed = edges * suppression.astype(np.uint8)
    edges_suppressed = cv2.morphologyEx(edges_suppressed, cv2.MORPH_CLOSE, np.ones((2, 2), np.uint8))
